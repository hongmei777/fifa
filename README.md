...
## Collecting data
We collected various tournament data from transfermarkt using python frameworks such as scrapy [1].
...
## Data exploration
The raw data which we collected for a single match is distributed over three files (match result, team lineups, historical data). We merge all files to one in which each row represents one game [1]. One row contains information such as
...
* weighted average of goals scored in past encounters
...
## References
The scraper and other data preprocessing files can be found [here](https://github.com/ViviLearns2Code/ml-fifa). 